...
bool MixxxApplication::notify(QObject* pTarget, QEvent* pEvent) {
    ...
    // and the NotificationStrip instead. Exceptions are DlgPreferences
    // (and its child dialogs) and the explicitly marked Assist control dialogs.
    switch (pEvent->type()) {
    case QEvent::ToolTip:
    ...
    case QEvent::Show:
        if (auto* pDialog = qobject_cast<QDialog*>(pTarget)) {
            if (!belongsToPreferencesDialog(pDialog) &&
                    !pDialog->property("bitedjAssistDialog").toBool()) {
